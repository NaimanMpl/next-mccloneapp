...
class UserMiddleware {
  public handleRegister = (user: RegisterFormData): Response | null => {
    if (
      ...
      return new Response(
        JSON.stringify({ message: 'Veuillez renseigner tout les champs.' }),
        { status: 400 }
      );
    }
  ...
  public handleUserCreation = (user: AddUserFormData) => {
    ...
    if (!(user.role in RoleEnum)) {
      return new Response(JSON.stringify({ message: 'Role inconnu' }), {
        status: 400,
      });
    }
    ...
    if (typeof user.admin !== 'boolean') {
      return new Response(
        JSON.stringify({ message: 'Le champ admin doit être un booleen' }),
        { status: 400 }
      );
    }
  ...
  public handlePatch = (searchParams: URLSearchParams) => {
    ...
    if (!email && !username) {
      return new Response(
        JSON.stringify({ message: 'Veuillez saisir au moins un champ !' }),
        { status: 400 }
      );
    }
    ...
    if (username) {
      if (username.length < 2 || username.length > 20) {
        return new Response(
          JSON.stringify({
            message:
              "Veuillez saisir un nom d'utilisateur dont la taille est comprise entre 2 et 20",
          }),
          { status: 400 }
        );
      }
